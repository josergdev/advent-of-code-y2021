...
execute bs i  = filter won (head $ winner (run bs i))

run2 bs = scanl (\(acci, accbs) i -> (i, markN accbs i)) (0, bs)

run22 = filter (\(n,bs) -> wonN bs)

sol1 = score $ head $ run22 $ run2 boards input

execute22 = snd3 $ head $ lastlastwinner $ winnersMapped (run22 (run2 boards input))

execute222 = toSimpleBoardN (snd execute22)

execute33 = snd3 $ head $ lastwinner $ winnersMapped (run22 (run2 boards input))

execute333 = toSimpleBoardN (snd execute33)

winnerSimpleBoard = head $ filter (`notElem` execute222) execute333

execute3333 = Data.Bifunctor.second (filter (\b -> toSimpleBoard b == winnerSimpleBoard)) execute33

sol2 = score execute3333


winnersMapped = map (\(n,bs) -> (n,(n, filter won bs),bs))

lastwinner = filter (\(n,(wn, wbs),bs) -> length wbs == length bs)

lastlastwinner :: [(a1, (a2, [a3]), [a4])] -> [(a1, (a2, [a3]), [a4])]
lastlastwinner = filter (\(n,(wn, wbs),bs) -> length wbs == (length bs - 1))

toSimpleBoardN = map toSimpleBoard

_toSimpleBoard = map (map fst)

toSimpleBoard (Board m) = _toSimpleBoard m
